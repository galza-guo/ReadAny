...
function getAutoTranslateNextPageLabel(value: number) {
	const option = AUTO_TRANSLATE_NEXT_PAGE_OPTIONS.find(
...
function ProviderIcon({
	...
	return (
		<ProviderBrandIcon className={className} providerKind={providerKind} />
	);
}
...
function ModelCombobox({
	...
	return (
		<div
			className={`model-combobox ${showPanel ? "is-open" : ""}`}
			ref={rootRef}
		>
			<input
				autoComplete="off"
...
function getModelLoadHint(preset: TranslationPreset, apiKeyInput: string) {
	if (
		providerUsesEditableBaseUrl(preset.providerKind) &&
		!preset.baseUrl?.trim()
	) {
	...
	if (
		providerUsesApiKey(preset.providerKind) &&
		!apiKeyInput.trim() &&
...
export function SettingsDialogContent({
	...
	const appLanguageSearchPlaceholder = hasLocaleMessage(
		...
		: t("languages.search");
	const translateToSearchPlaceholder = hasLocaleMessage(
		"languages.searchOrCustom",
	)
		? t("languages.searchOrCustom")
		: t("languages.search");
	const { showToast } = useToast();
	void apiKeyEditingPresetId;
	void presetModelMessages;
	...
	const [testButtonExpandedId, setTestButtonExpandedId] = useState<
		string | null
	>(null);
	...
	const [fetchButtonExpandedId, setFetchButtonExpandedId] = useState<
		string | null
	>(null);
	const fetchButtonTimersRef = useRef<
		Record<string, { collapseTimerId?: number }>
	>({});
	const prevFetchModelsRef = useRef<Record<string, boolean>>({});
	const previousSaveStateByIdRef = useRef<
		Record<string, PresetSaveStatus["state"] | undefined>
	>({});
	...
	const editingPresetShowsBaseUrlField = editingPreset
		...
		: false;
	const editingPresetApiKeyState =
		editingPreset && editingPresetShowsApiKeyField
			? getPresetApiKeyFieldState({
					apiKeyConfigured: editingPreset.apiKeyConfigured,
					apiKeyInput: editingPresetApiKeyInput,
				})
			: undefined;
	const editingPresetModels = editingPreset
		? (presetModels[editingPreset.id] ?? [])
		: [];
	...
	useEffect(() => {
		settings.presets.forEach((preset) => {
			...
			const currentState = (presetSaveStatusById[presetId]?.state ??
				"pristine") as PresetSaveStatus["state"];
			...
			if (
				...
				showToast({ message: t("toast.presetSaved"), tone: "success" });
			} else if (
				currentState === "error" &&
				(previousState === "dirty" || previousState === "saving")
			) {
				showToast({
					message: t("toast.presetSaveFailed"),
					tone: "error",
					actionLabel: t("settings.retrySave"),
					onAction: () => {
						void Promise.resolve(onRetryPresetSave(presetId)).catch(() => {});
					},
				});
			}
	...
	}, [presetSaveStatusById, settings.presets, showToast, onRetryPresetSave]);
			...
			if (wasRunning && testStatus) {
				...
				if (testStatus?.ok) {
					...
					const timer = window.setTimeout(() => {
						setTestButtonExpandedId((current) =>
	...
	useEffect(() => {
		settings.presets.forEach((preset) => {
			...
			if (wasLoading && !isLoading && editingPreset?.id === preset.id) {
				...
				const timer = window.setTimeout(() => {
					setFetchButtonExpandedId((current) =>
						current === preset.id ? null : current,
					);
	...
	return (
		<Tooltip.Provider delayDuration={250}>
				...
				<Tabs.Content className="settings-content" forceMount value="general">
					<div className="settings-layout">
						...
						<div className="settings-block settings-block-inline">
							<Label.Root
								className="settings-label type-field-label"
								htmlFor="auto-translate-next-pages"
							>
						...
						<div className="settings-block settings-block-inline">
							...
							<div className="settings-inline-control">
								<div className="accent-color-picker" id="accent-color-picker">
						...
						<div className="settings-block settings-block-providers">
							<div className="settings-toolbar">
								<div className="settings-toolbar-heading">
									<span className="settings-toolbar-title type-section-title">
														...
														<Fragment key={provider.value}>
															...
															<button
																...
																onClick={() => {
																	...
																	if (presetId) {
																		void Promise.resolve(
																			onEditingPresetChange(presetId),
																		).catch(() => {});
																	}
																}}
																type="button"
															>
																...
																/>
																<span className="settings-provider-option-label">
																	{provider.label}
																</span>
															</button>
														</Fragment>
							...
							{settings.presets.length === 0 ? (
								...
								<div className="settings-preset-list">
									{settings.presets.map((preset) => {
										...
										const validation = getPresetValidationState(
											preset,
											apiKeyInput,
										);
										const testStatus = presetStatuses[preset.id];
												...
												<div className="settings-preset-row">
													...
													>
														<div className="settings-preset-copy">
															<div className="settings-preset-title-row settings-preset-title-row--editable">
																<span className="settings-preset-label type-pane-title">
																	{preset.customLabel || preset.label}
																</span>
																<button
																	aria-label={t("common.rename")}
																	className="settings-preset-rename-btn"
																	onClick={(e) => {
																		e.stopPropagation();
													...
													<div className="settings-preset-editor">
																...
																<Select.Root
																	...
																	onValueChange={(value) =>
																		onPresetChange({
																			...editingPreset,
																			providerKind:
																				value as TranslationProviderKind,
																		})
																	}
																>
																	<Select.Trigger
															...
															{editingPresetShowsApiKeyField ? (
																<div className="settings-editor-row">
																	...
																	<input
																		id="preset-api-key"
																		className={
																			editingPresetApiKeyState?.showsSavedMask
																				? "input input-masked"
																				: "input"
																		}
																		type={
																		...
																		placeholder={
																			editingPresetApiKeyState?.placeholder
																		}
																		value={
																			editingPresetApiKeyState?.displayValue ??
																			""
																		}
																		onBlur={() => {
																			void Promise.resolve(
																				onPresetApiKeyBlur(editingPreset.id),
																			).catch(() => {});
																		}}
																		onChange={(event) => {
																		...
																		onFocus={() =>
																			onPresetApiKeyFocus(editingPreset.id)
																		}
																	/>
																</div>
															) : null}
															...
															providerUsesCodingPlan(
																...
																<div className="settings-editor-row settings-editor-row--toggle">
																	...
																	>
																		<div className="settings-toggle-copy">
																			<div className="settings-toggle-title-row">
																				<span className="settings-toggle-title">
																					{t("settings.codingPlan")}
																				</span>
																				<Tooltip.Root>
																					<Tooltip.Trigger asChild>
																						<button
																							aria-label={t(
																								"settings.aboutCodingPlan",
																							)}
																							className="btn btn-icon-only btn-quiet-action settings-help-button"
																							type="button"
																						>
																		...
																		<button
																			aria-checked={
																				editingPreset.codingPlan ?? false
																			}
																	...
																	<Select.Root
																		...
																		onValueChange={(value) =>
																			onPresetChange({
																				...editingPreset,
																				thinking:
																					value as ProviderReasoningMode,
																				reasoning: undefined,
																			})
																		}
																	>
																...
																<div className="settings-editor-row">
																	...
																	>
																		...
																		<Select.Portal>
																			...
																			>
																				<Select.Viewport>
																					{STANDARD_REASONING_OPTIONS.map(
																						(option) => (
																							<Select.Item
																								key={option.value}
																								value={option.value}
														...
														<div className="settings-actions-row">
															<ExpandableIconButton
																...
																className="settings-icon-button"
																disabled={Boolean(
																	presetModelsLoadingById[editingPreset.id],
																)}
																expanded={
																	Boolean(
																		presetModelsLoadingById[editingPreset.id],
																	) ||
																	fetchButtonExpandedId === editingPreset.id
																}
																label={
																	presetModelsLoadingById[editingPreset.id]
																		? editingPresetModels.length > 0
																			? t("settings.reloadingModels")
																...
																labelDirection="left"
																onClick={() => {
																	if (!editingPresetCanLoadModels) {
																		showToast({
																			message: getModelLoadHint(
																				editingPreset,
																				editingPresetApiKeyInput,
																			),
																			tone: "neutral",
																		});
																		return;
																	...
																	void Promise.resolve(
																		onFetchPresetModels(editingPreset.id),
																	).catch(() => {});
																}}
															>
															...
															<ExpandableIconButton
																...
																disabled={
																	presetTestRunningId === editingPreset.id ||
																	!editingPresetValidation?.isValid
																}
																expanded={
																	presetTestRunningId === editingPreset.id ||
																	testButtonExpandedId === editingPreset.id
																}
																label={
																	presetTestRunningId === editingPreset.id
															...
															<ExpandableIconButton
																...
																labelDirection="left"
																onClick={() =>
																	setPendingDeletePresetId(editingPreset.id)
																}
															>
														...
														{presetStatuses[editingPreset.id] &&
														!presetStatuses[editingPreset.id]?.ok ? (
							...
							<div className="settings-provider-toggle-group">
								<div className="settings-toggle-row">
									<div className="settings-toggle-copy">
										<div className="settings-toggle-title-row">
											<span className="settings-toggle-title">
												{t("settings.translateAllSlowMode")}
											</span>
								...
								<div className="settings-toggle-row">
									<div className="settings-toggle-copy">
										<div className="settings-toggle-title-row">
											...
											<span
												aria-hidden="true"
												className="settings-experimental-badge"
											>
							...
							<div className="settings-cache-summary-copy">
								...
								<span className="settings-cache-summary-value type-pane-title">
									{translationCacheLoading && translationCacheSummary === null
										? t("common.loading")
										: formatCacheSize(
												translationCacheSummary?.totalCacheSizeBytes ?? 0,
											)}
								</span>
							</div>
							<ExpandableIconButton
							...
							<ul className="settings-cache-list">
								{cacheBooks.flatMap((book) =>
									book.languages.map((language) => {
										...
										const isDeletingBook =
											translationCacheActionTarget === actionKey;

										return (
											<li key={actionKey} className="settings-cache-item">
												<span className="settings-cache-item-icon">
													<FileIcon kind={book.fileType === "epub" ? "epub" : "pdf"} size={18} />
												</span>
												<div className="settings-cache-item-copy">
													<span
														className="settings-cache-item-title type-pane-title"
														title={book.title}
													>
														{book.title}
													</span>
													<span className="settings-cache-item-detail type-meta">
														...
														{language.isLegacyOnly
															? t("cache.legacyCachedPages", {
																	count: String(language.cachedPageCount),
																})
															: t("cache.cachedPages", {
																	count: String(language.cachedPageCount),
																})}
												...
												<ExpandableIconButton
													aria-label={t("settings.deleteCachedPagesFor", {
														title: book.title,
													})}
													className="settings-icon-button settings-icon-button-danger"
													disabled={cacheActionInProgress}
			...
			<ConfirmationDialog
				...
				description={
					pendingDeletePreset
						? t("dialog.deletePresetDescription", {
								label: pendingDeletePreset.label,
							})
						: ""
				}
				onOpenChange={(open) => {
